...
module.exports = {
    ...
    connect: async function (callback) {
        if (!this.db) {
            let { url, username, password, host, port, database, name, connector, debug, ...validOptions } = this.options;
            if (!this.options.url) {
    ...
    delete: async function (where, countFiles = false) {
        ...
        if (files && files.length) {
            ...
            filesDeleted = [...files.reduce((map, item) => { if(item?.filename) { map.has(item.filename) || map.set(item.filename, item); } return map; }, new Map()).values()].length;
        }
        const result = {}
        if (countFiles) {
