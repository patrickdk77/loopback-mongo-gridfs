...
module.exports = {
    ...
    fileDownload(ctx, file, res, inline = false, namePattern = '{$filename}') {
        ...
        res.set('Content-Disposition', `${(inline) ? 'inline' : 'attachment'};filename=`+encodeURIComponent(this.replaceInPattern(namePattern, file)).replace(/[!'()*]/g,(c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`,));
    ...
    zipDownload(ctx, files, res, zipName = 'files', namePattern = '{$filename}') {
        ...
        res.set('Content-Disposition', `attachment;filename=`+encodeURIComponent(zipName+'.zip').replace(/[!'()*]/g,(c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`,));
        return archive;
    },
    convertWhere(where) {
        const query = {};
