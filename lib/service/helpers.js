...
module.exports = {
    ...
    fileDownload(ctx, file, res, inline = false, namePattern = '{$filename}') {
        ...
        res.set('Content-Disposition', `${(inline) ? 'inline' : 'attachment'};filename="${this.replaceInPattern(namePattern, file)}"; filename*=UTF-8''`+encodeURIComponent(this.replaceInPattern(namePattern, file)).replace(/['()*]/g,(c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`,).replace(/%(7C|60|5E)/g, (str, hex) => String.fromCharCode(parseInt(hex, 16)),));
    ...
    zipDownload(ctx, files, res, zipName = 'files', namePattern = '{$filename}') {
        ...
        res.set('Content-Disposition', `attachment;filename="${zipName}.zip"; filename*=UTF-8''`+encodeURIComponent(`${zipName}.zip`).replace(/['()*]/g,(c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`,).replace(/%(7C|60|5E)/g, (str, hex) => String.fromCharCode(parseInt(hex, 16)),));
        return archive;
    },
    convertWhere(where) {
        const query = {};
